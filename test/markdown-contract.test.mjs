...
test("compiled package maps core Markdown through documented Obsidian variables", async () => {
  ...
  const expectedSemanticMappings = {
    ...
    "--blockquote-color": "var(--pixel-text)",
    "--blockquote-background-color": "color-mix(in srgb, var(--pixel-amber) 6%, var(--pixel-paper))",
    "--text-highlight-bg": "var(--pixel-context-label)",
    "--tag-color": "var(--pixel-tag-ink)",
    "--tag-background": "var(--pixel-tag-fill)",
    "--tag-border-color": "var(--pixel-tag-edge)",
...
test("active editing lines use an offset pixel-paper strip without a start rule", async () => {
  const css = await readTheme();
  const activeLine = ruleBody(
    css,
    ":where(.markdown-source-view.mod-cm6) .cm-line.cm-active",
  );
  ...
  assert.doesNotMatch(
    activeLine,
    /(?:\binset\b|border-(?:left|inline-start))\s*:/,
  );
});
